...
Write-Host($pipeline)
# Update the assigned_to variable with the user's ameuniqueName
# $pipeline.variables.assigned_to.value = "702"
...
Invoke-RestMethod -Uri $patchUrl -Method POST -Body $json_payload -ContentType "application/json" -Headers @{Authorization = "Bearer $SYSTEM_ACCESSTOKEN"}
