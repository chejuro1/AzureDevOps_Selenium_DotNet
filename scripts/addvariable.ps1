...
$response = Invoke-RestMethod -Uri "$SYSTEM_TEAMFOUNDATIONSERVERURI$SYSTEM_TEAMPROJECTID/_apis/distributedtask/variablegroups?api-version=7.1-preview.2" -Headers @{Authorization = "Bearer $SYSTEM_ACCESSTOKEN"}
...
Invoke-RestMethod -Uri "${SYSTEM_TEAMFOUNDATIONSERVERURI}${SYSTEM_TEAMPROJECTID}/_apis/distributedtask/variablegroups/${group_id}?api-version=7.1-preview.2" -Method Put -Body $json -ContentType "application/json" -Headers @{
